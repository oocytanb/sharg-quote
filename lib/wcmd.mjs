...
import { emptyElementNotAllowedMessage, validateNotEmpty, } from './xcommand.mjs';
...
export function customQuote(pred) {
    return (a) => {
        if (a.length === 0) {
            ...
            const b = a.replace(/\n/g, ' ');
            const c = pred(b) || /[" \t|<>&()[\]{}^=;!'+,`~]/.test(b) ? wargQuote(b) : b;
 ...
 * @internal
 */
export function customCommand(shellOptions) {
